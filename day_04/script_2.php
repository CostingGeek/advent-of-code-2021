...
function check_winner( $score_list, $winner_list ) {
    
    $winner_tmp = $winner_list;
    foreach( $score_list as $key_1 => $score_grid ) {
        if( in_array( $key_1, $winner_list ) )
        {
            continue;
        }

        $col_score = array(0,0,0,0,0);
        ...
        foreach( $score_grid as $key_2 => $score_line ) {
            ...
            foreach( $score_line as $key_3 => $score ) {
                $col_score[$key_3] += $score;

                if( $col_score[$key_3] == 5 ) {
                    $winner_tmp[] = $key_1;
                }
            }
        }
...
foreach( $draw_array as $draw ) {
    ...
    $winner_list = check_winner( $score_list, $winner_list );

    if( count( $winner_list ) == count( $grid_list ) ) {
        break;
    }
}

$winner = end( $winner_list );

$grid_sum = unmarked_numbers( $grid_list[$winner], $score_list[$winner] );

echo "\n\r".'Last Winning Grid: '.$winner;
